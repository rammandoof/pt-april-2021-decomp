function scr_pizzagoblin_throw()
{
    if (!variable_global_exists("throw_frame"))
    {
        ...
        global.reset_timer[obj_noisegoblin] = 200 / (global.stylethreshold + 1);
        global.reset_timer[obj_cheeserobot] = 200 / (global.stylethreshold + 1);
        global.reset_timer[obj_spitcheese] = 100 / (global.stylethreshold + 1);
        global.reset_timer[obj_trash] = 100;
        global.reset_timer[obj_invtrash] = 100 / (global.stylethreshold + 1);
        global.reset_timer[obj_robot] = 200 / (global.stylethreshold + 1);
        global.reset_timer[obj_kentukykenny] = 200 / (global.stylethreshold + 1);
        global.reset_timer[obj_pizzard] = 100 / (global.stylethreshold + 1);
        global.reset_timer[obj_pepgoblin] = 200 / (global.stylethreshold + 1);
        global.reset_timer[obj_swedishmonkey] = 200 / (global.stylethreshold + 1);
        global.reset_timer[obj_rancher] = 100 / (global.stylethreshold + 1);
        global.reset_timer[obj_pickle] = 200 / (global.stylethreshold + 1);
        global.reset_timer[obj_tank] = 100 / (global.stylethreshold + 1);
    ...
    if (bombreset <= 0 && floor(image_index) == global.throw_frame[object_index])
    {
        ...
        switch (object_index)
        {
            ...
            case obj_smokingpizzaslice:
                substate = choose(states.enemy_walk, states.enemy_idle, states.enemy_idle);
