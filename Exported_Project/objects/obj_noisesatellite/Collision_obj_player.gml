if (state != states.grabbed)
{
    with (obj_player)
    {
        ...
        if (attacking == 0 && (state != states.tackle && state != states.hurt) && !(y < other.y) && grabbing == 0 && other.state != states.enemy_stun)
        {
            if (x != other.x)
            {
                other.image_xscale = -sign(other.x - x);
            ...
            if (other.state == states.enemy_walk || other.state == states.enemy_turn)
                other.state = states.enemy_idle;
            
            image_index = 0;
            state = states.bump;
            
            if (other.state == states.enemy_land)
                other.state = states.enemy_idle;
        }
    }
}
