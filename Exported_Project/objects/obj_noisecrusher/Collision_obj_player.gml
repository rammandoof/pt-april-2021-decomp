if (invframes == 0)
{
    if (state != states.grabbed && obj_player.state != states.boxxedpep && obj_player.state != states.bump)
    {
        with (obj_player)
        {
            if (y > other.y && other.vsp > 1 && other.grounded == 0 && other.state == states.enemy_bounce)
            ...
            if ((state == states.mach2 || state == states.mach3 || state == states.grab) && other.grounded == 1 && other.state == states.enemy_stun)
            {
                global.hit += 1;
            ...
            if (!(y < other.y) && state != states.bump && state != states.hurt && other.state != states.enemy_stun)
            {
                instance_create(x, y, obj_bumpeffect);
