...
if (sprite_index != spr_haystackburning && sprite_index != spr_haystackburningup)
{
    with (obj_peasanto)
    {
        if (state == states.enemy_charge && place_meeting(x + hsp, y, other))
        ...
        if (state != states.fireass && state != states.knightpep)
        {
            ...
            else if (scr_transformationcheck())
            {
                ...
                if (state != states.fireass)
                    tv_push_prompt_once(tv_create_prompt("This is the fireass transformation text", tvprompts.transfo, spr_tv_fireass, 3), "fireass");
